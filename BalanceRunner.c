...
void BalanceRunner_run(BalanceRunner* this, int turn, int forward)
{
	...
	*/
	...
	if(turn != 0){
		...
		//spin_count_l = Motor_getAngle(this->leftMotor) - motor_ang_l;
		//spin_count_r = Motor_getAngle(this->rightMotor) - motor_ang_r;
		if(motor_ang_l < motor_ang_r){
			Motor_rotate(this->leftMotor,pwm_l + 1);
			Motor_rotate(this->rightMotor,pwm_r);
		}else if(motor_ang_l > motor_ang_r){
			Motor_rotate(this->leftMotor,pwm_l);
			Motor_rotate(this->rightMotor,pwm_r + 3);
		}else{
			Motor_rotate(this->leftMotor,pwm_l);
			Motor_rotate(this->rightMotor,pwm_r);
		}
